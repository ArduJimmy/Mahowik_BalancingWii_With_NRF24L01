...
www.multiwii.com
November  2013     V2.3configureReceiver();
...
//#endif
 ...
 //*********************************** 
  ...
  static float tspeed = 0.0f;
  tspeed = constrain(tspeed + ((float)acceleration * (float)cycleTime * 0.000001f), -MAX_SPEED, MAX_SPEED); 
    ...
    if(currAbsAngle < 250) {  // if angle less than 25 degree
      ...
      if(f.RISE_MODE) { // if robot fell, use it to auto rise! ;)
        ...
        if(risePhase == 0) { // get direct acceleration 
          ...
          tspeed = (currAngle > 0) ? riseSpeed : -riseSpeed; // forward direction
          ...
          tspeed = (currAngle > 0) ? -riseSpeed : riseSpeed; // backward direction
        ...
        tspeed = constrain(-targetSpeed/2, -MAX_SPEED/2, MAX_SPEED/2);
  ...
  int16_t outputSpeed = constrain(tspeed * dynK, -MAX_SPEED, MAX_SPEED); ;
